...
目录缺省是**包所在的 app/** 下的 characters / scenes / libraries（与 gui/app.py 同一
口径：按 `__file__` 解析，故在任意 cwd 下、双击或模块启动都命中同一个素材库）。
"""
...
#: app/ 根（characters/scenes/libraries 所在目录）——与 gui/app.py 同一算法，任意 cwd 都命中。
_APP_DIR = Path(__file__).resolve().parents[3]
DEFAULT_CHARACTERS_DIR = _APP_DIR / "characters"
DEFAULT_SCENES_DIR = _APP_DIR / "scenes"
DEFAULT_LIBRARIES_DIR = _APP_DIR / "libraries"
