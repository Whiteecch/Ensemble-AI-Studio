@echo off
rem Ensemble-AI-Studio —— 交互 Demo 启动器（双击即可；可加参数，如 --stub 离线、--pace auto）
chcp 65001 >nul
cd /d "%~dp0"
...
rem 2) 明确告诉用的是哪个后端，别再看到占位台词还不清楚原因。
...
rem 3) 没建虚拟环境时给出可照做的提示（公开仓库最常见的第一次失败）。
...
".venv\Scripts\python.exe" -m harness.runner --demo --pace enter %*
echo.
pause
